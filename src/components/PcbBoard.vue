<template>
  <div ref="PCB" class="pcb vdr filler inactive" 
    :style="{width: w+'mm', height: h+'mm', filter: brightness, backgroundColor: color}"  @click="resize_show()">
    <div class="vdr-stick vdr-stick-mr"  @mousedown="resizeVr_init($event)" 
    style="width: 8px; height: 8px; margin-top: -4px; right: -4px;">
    </div>
    <div id="mm_rule" class="vdr-stick" style="height:100mm;width:100mm;display:none"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    resize_hide(e) {
      ...
      if (!e.target.classList.contains("pcb") && element.classList.contains("active")) {
    ...
    resizeVr_init(e) {
      ...
      // Add listeners
      document.documentElement.addEventListener('mousemove', this.resizeVr_drag, false);
      document.documentElement.addEventListener('mouseup', this.resizeVr_drop, false);
    ...
    resizeVr_drop(e) {
      ...
      // Remove listeners
      document.documentElement.removeEventListener('mousemove', this.resizeVr_drag, false);
      document.documentElement.removeEventListener('mouseup', this.resizeVr_drop, false);
      // Send event
